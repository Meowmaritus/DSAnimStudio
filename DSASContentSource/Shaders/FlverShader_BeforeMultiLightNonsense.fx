...
float SSS_Test_Intensity_Mask = 3;
...
float4 GetMainColor(VertexShaderOutput input, bool isFrontFacing : SV_IsFrontFace) : COLOR
{
    ...
    else if (WorkflowType == WORKFLOW_PBR_GLOSS)
    {
        ...
        float LdotN_ForDiffuse = (pow(LdotN,LdotNPower*1))*1 / PI;
        
        float3 finalDiffuse;
        [branch]
        if (EnableSSS)
        {
            float sssMask = saturate(mask3.r * SSS_Test_Intensity_Mask);
            
            [branch]
            if (EnableSSS_EnergyConserving)
            {
                float3 SSS_Wrap_Add = (float3(1,1,1) - (SSSColor.rgb * (sssMask * SSSIntensity * 0.99))) * SSS_Test_Intensity_Add;
                float3 SSS_Wrap_Exp = ((float3(1,1,1) / SSS_Wrap_Add) - float3(1,1,1)) * SSS_Test_Intensity_Exp;
                
                finalDiffuse.r = pow( max(0.0, (LdotN_ForDiffuse) + SSS_Wrap_Exp.r) * SSS_Wrap_Add.r, 1.0 + SSS_Wrap_Exp.r)
                               * (2.0 + SSS_Wrap_Exp.r) * (SSS_Wrap_Add.r * 0.5);
                               
                finalDiffuse.g = pow( max(0.0, (LdotN_ForDiffuse) + SSS_Wrap_Exp.g) * SSS_Wrap_Add.g, 1.0 + SSS_Wrap_Exp.g)
                               * (2.0 + SSS_Wrap_Exp.g) * (SSS_Wrap_Add.g * 0.5);
                finalDiffuse.b = pow( max(0.0, (LdotN_ForDiffuse) + SSS_Wrap_Exp.b) * SSS_Wrap_Add.b, 1.0 + SSS_Wrap_Exp.b)
                               * (2.0 + SSS_Wrap_Exp.b) * (SSS_Wrap_Add.b * 0.5);
            ...
            else
            {
                ...
                finalDiffuse.r = saturate((LdotN_ForDiffuse) * (1.0 - sssWrap.r) + sssWrap.r);
                finalDiffuse.g = saturate((LdotN_ForDiffuse) * (1.0 - sssWrap.g) + sssWrap.g);
                finalDiffuse.b = saturate((LdotN_ForDiffuse) * (1.0 - sssWrap.b) + sssWrap.b);
            }
        ...
        {
            finalDiffuse = diffuseColor * LdotN_ForDiffuse;
        }

    ...
    public static class GraphicsExtensions
    {
       ...
       public static TexCoordPointerType OpenGLTexCoordPointerType(this VertexElementFormat elementFormat)
        ...
        public static bool IsCompressedFormat(this SurfaceFormat format)
        {
            if (format.IsDXGI())
            {
                var dxgiFormat = format.GetDXGI();
                return SharpDX.DXGI.FormatHelper.IsCompressed(dxgiFormat);
            }
        ...
        public const int DXGI_FORMAT_START = 1000;

        public static bool IsDXGI(this SurfaceFormat surfaceFormat)
        {
            return (int)surfaceFormat >= DXGI_FORMAT_START;
        }

        public static SharpDX.DXGI.Format GetDXGI(this SurfaceFormat surfaceFormat)
        {
            return (SharpDX.DXGI.Format)((int)surfaceFormat - DXGI_FORMAT_START);
        }

        public static int GetSize(this SurfaceFormat surfaceFormat)
        {
            if (surfaceFormat.IsDXGI())
            {
                var dxgiFormat = surfaceFormat.GetDXGI();

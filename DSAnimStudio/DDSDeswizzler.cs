    ...
    public class DDSDeswizzler
    {
        ...
        public void DeswizzleDDSBytesPS3(int width, int height, int offset, int offsetFactor)
        {
            if (width * height > 4)
            {
                ...
                DeswizzleDDSBytesPS3(width / 2, height / 2, offset + ((width / 2) * (height / 2) * offsetFactor), offsetFactor * 2);
                DeswizzleDDSBytesPS3(width / 2, height / 2, offset + ((width / 2) * (height / 2) * offsetFactor) + (width / 2), offsetFactor * 2);
            }
            else
            {
        ...
        public void DeswizzleDDSBytesPS4(int width, int height, int offset, int offsetFactor)
        {
            if (width * height > 16)
            {
                ...
                DeswizzleDDSBytesPS4(width / 2, height / 2, offset + ((DDSWidth / 8) * (height / 4) * BlockSize), offsetFactor * 2);
                DeswizzleDDSBytesPS4(width / 2, height / 2, offset + (DDSWidth / 8) * (height / 4) * BlockSize + (width / 8) * BlockSize, offsetFactor * 2);
            }
            else
            {
        ...
        {
            int numStripes = width / 32;
            int numBlocksInStripe = height / 32;
            int ddsBlockSize = 8;
            int stripeWidth = ddsBlockSize * 8;
            int stripeBlockSize = stripeWidth * 8;
            int offset = 0;
            DDSWidth = 256;
            for (int i = 0; i < numStripes; i++)
            {
                for (int j = 0; j < numBlocksInStripe; j++)
                {
